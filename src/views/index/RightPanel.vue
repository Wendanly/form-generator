    ...
    <div class="field-box">
      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px">
          ...
          <el-form-item
            v-if="activeData.__config__.layout==='rowFormItemWrap'&&activeData.gutter!==undefined"
            label="栅格间隔"
          >
            <el-input-number v-model="activeData.gutter" :min="0" placeholder="栅格间隔" />
          </el-form-item>
          <el-form-item
            v-if="activeData.__config__.layout==='rowFormItemWrap'&&activeData.type!==undefined"
            label="布局模式"
          >
          ...
          <template v-if="['el-table'].indexOf(activeData.__config__.tag) > -1">
            ...
            <el-form-item style="margin-top: 20px;" label="添加列" label-width="60px">
              <el-select v-model="colType" style="width: 100px;" placeholder>
                <el-option
                  v-for="item in colTypeList"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
              </el-select>
              <el-button style="padding-bottom: 0" type="text" size="mini" @click="addTableItem">确定</el-button>
            </el-form-item>
            <el-divider />
          </template>
          <!-- 结束 -->
          ...
          <!-- activeData.__config__.optionType 多选框组，只有当optionType: 'button'时才有大小一说 -->
          <el-form-item
            v-if="activeData.size !== undefined &&
              (activeData.__config__.optionType === 'button' ||
                ...
                activeData.__config__.tag === 'el-select' ||
                activeData.__config__.tag === 'el-cascader' ||
                activeData.__config__.tag === 'el-time-picker' ||
          ...
          <!-- 分页组件 -->
          <el-form-item v-if="activeData.__config__.tag === 'el-pagination'" label="组件尺寸">
            <el-radio-group v-model="activeData.small">
              <el-radio-button :label="false">默认</el-radio-button>
              <el-radio-button :label="true">较小</el-radio-button>
            </el-radio-group>
          </el-form-item>
          <el-form-item v-if="activeData.__config__.tag === 'el-pagination'" label="背景色">
            <el-switch v-model="activeData.background" />
          </el-form-item>
          <el-form-item v-if="activeData.__config__.tag === 'el-pagination'" label="按钮数量">
            <el-input-number v-model.number="pagerCount" :min="5" :max="21" placeholder="请输入按钮数量" />
          </el-form-item>
          <el-form-item
            ...
            label-width="100px"
          >
            <el-input v-model="activeData.__config__.forData.rowsKey" placeholder="请输入rows字段名"></el-input>
          </el-form-item>
          <el-form-item
            v-if="activeData.__config__.tag === 'el-pagination'"
            label="page字段名"
            label-width="100px"
          >
            <el-input v-model="activeData.__config__.forData.pageKey" placeholder="请输入page字段名"></el-input>
          </el-form-item>
          <el-form-item
            v-if="activeData.__config__.tag === 'el-pagination'"
            label="总条数字段名"
            label-width="120px"
          >
            <el-input v-model="activeData.__config__.forData.totalKey" placeholder="请输入字段名"></el-input>
          </el-form-item>
          <!-- <el-form-item v-if="activeData.__config__.tag === 'el-pagination'" label="布局">
            <el-checkbox-group v-model="tmpLayout">
              <el-checkbox v-for="(item,index) in layoutList" :key="index" :label="item">{{item}}</el-checkbox>
            </el-checkbox-group>
          </el-form-item>-->
...
export default {
  ...
  computed: {
    //分页
    // layoutList() {
    //   return this.activeData.layout.split(",");
    // },
    // tmpLayout() {
    //   return JSON.parse(JSON.stringify(this.layoutList || []));
    // },
    // 表格
    currentBtnList() {
      if (this.activeData.__config__.tag != "el-table") return [];
      let arr = [];
  ...
  watch: {
    ...
    tmpLayout(val) {
      ...
      arr.indexOf(val);
      this.activeData.layout = val.length ? val.join(",") : "";
    },
    pagerCount(val) {
      console.log(val);
      return (this.activeData["pager-count"] = val);
    }
  },
  methods: {
    setBtn(val) {
    ...
    addTableItem() {
      ...
      switch (this.colType) {
        case "1":
          ...
          if (index == -1) {
            this.activeData.__config__.children.push(obj); //无操作列，则直接push
          } else {
            this.activeData.__config__.children.splice(index, 0, obj); //在操作列之前添加
          }
          break;
        case "4":
    ...
    setDefaultValue(val) {
      ...
      //   return val
      // }
      //输入框只能接受字符串和数字，布尔值不接受，所以需要转换成字符串
